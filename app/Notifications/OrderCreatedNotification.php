...
class OrderCreatedNotification extends Notification
{
    ...
    public function toMail($notifiable)
    {
        ...
        return (new MailMessage)
                    ->subject('New Odrder' . $this->order->number) // OrderCreatrdNotification By Default
                    ->from('notification@vendor-store.ps', 'Store Vendor') // By Default Use .env settings
    ...
    {
        $addr = $this->order->billingAddress;
        return [ // store in database in data fill with json type
    ...
    {
        $addr = $this->order->billingAddress;
        return new BroadcastMessage([ // store in database in data fill with json type
            'body' => "A new order (#{$this->order->number}) created by {$addr->name} from {$addr->country_name}.",
            'icon' => 'fas fa-file',
            'url' => url('dashboard'),
            'order_id' => $this->order->id,
